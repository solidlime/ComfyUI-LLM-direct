...
- [x] T003：`__init__.py` の gguf-llm-direct をヘルパー使用に切替（挙動不変をテストで確認）
- [x] T004：`APILLMDirect` 実装（MockTransport 注入可能な設計）
- [x] T005：chat_completion テスト（200 / 400 / timeout / content=None / seed / api_key ヘッダー）
- [x] T006：ComfyUI 起動確認（venv_cu13 でノード登録を実機確認: GGUFLLMDirect + APILLMDirect）

## 優先度：中
- [x] T007：README 更新（api-llm-direct の使い方、env var 推奨、seed 注記）
...
- [x] T011：APILLMDirect に入力 2 つ追加（generate 配線含む）
...
- [x] T016：APILLMDirect の generate を stream 化 + send_progress_text 配線
- [x] T017：GGUFLLMDirect の generate を stream 化 + send_progress_text 配線（llama_cpp 0.3.34 は chat handler が stream 対応）
...
## 優先度：中（hf-llm-direct: transformers 直読みノード、2026-08-12）
- [x] T026：SPEC 更新（機能5 hf-llm-direct）+ #081 事前アーキテクチャ判断（CausalLM フィルタ列挙・スレッド例外伝播・bf16・遅延 import）
...
- [x] T029：HFLLMDirect 実装（CausalLM 列挙・1 モデルキャッシュ+gc+empty_cache・TextIteratorStreamer）+ web/llm-direct.js 対象追加 + tests/test_hf_client.py（Fake 注入）+ README 更新
- [x] T030：3 ノード名リネーム（GGUFLLMDirect / APILLMDirect / HFLLMDirect、表示名 -llm-direct 形式。完全置き換え・旧名エイリアスなし）
